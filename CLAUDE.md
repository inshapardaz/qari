...
- `DictionaryService` + provider interface (`dictionary.ts`) — orchestrates local (e.g. StarDict) → user-supplied → built-in online providers in that priority order; see README's Dictionary Integration section for the provider contract.
